...
import uuid
from backend.models import ProductTestConfig, ProductTestConfigCreate, User
from backend.database import db
from backend.routers.auth import get_current_user, get_current_active_analyst

router = APIRouter(prefix="/product-test-configs", tags=["product-test-configs"])


@router.get("/rollout")
async def get_trial_media_rollout(
    current_user: Annotated[User, Depends(get_current_user)],
):
    """Phase 7 — trial media rollout stage and enabled capabilities."""
    del current_user
    from backend.utils.trial_media_rollout_flags import get_trial_media_rollout_payload

    return get_trial_media_rollout_payload()


@router.post("/", response_model=ProductTestConfig)
async def create_config(
    config_in: ProductTestConfigCreate,
    current_user: Annotated[User, Depends(get_current_active_analyst)]
):
    configs_col = db.get_collection("product_test_configs")
...
@router.get("/", response_model=List[ProductTestConfig])
...
@router.get("/{config_id}", response_model=ProductTestConfig)
...
@router.put("/{config_id}", response_model=ProductTestConfig)
