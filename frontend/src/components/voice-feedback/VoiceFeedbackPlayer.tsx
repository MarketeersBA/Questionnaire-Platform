...
export default function VoiceFeedbackPlayer({ feedbackId, showControls = true }: VoiceFeedbackPlayerProps) {
    ...
    if (loading) {
        return (
            <div className="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-xl animate-pulse w-fit">
    ...
    return (
        <div className="flex items-center gap-3 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-2 pl-3 pr-4 rounded-2xl shadow-sm hover:shadow-md transition-all group">
            <audio
                ref={audioRef}
                src={audioUrl!}
            ...
            <div className="flex flex-col min-w-[80px]">
                <div className="flex items-center gap-1.5">
                    <Volume2 size={12} className={playing ? 'text-brand-blue animate-pulse' : 'text-slate-400'} />
                ...
                <div className="h-1 bg-slate-100 dark:bg-slate-800 rounded-full mt-1.5 relative overflow-hidden w-full">
                    {playing && (
                        <div className="absolute inset-0 bg-brand-blue/30 animate-pulse" />
                    )}
                </div>
            </div>
            ...
            {showControls && (
                <button
                    ...
                    className="p-2 text-slate-400 hover:text-brand-blue hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg transition-colors ml-auto opacity-0 group-hover:opacity-100"
