...
export function OpportunityCard({ opportunity }: { opportunity: OpportunityInsight }) {
    ...
    return (
        <div className={`relative overflow-hidden group bg-surface rounded-[32px] border ${isCritical ? 'border-rose-100 dark:border-rose-500/20 shadow-rose-500/5' : 'border-amber-100 dark:border-amber-500/20 shadow-amber-500/5'} border-solid shadow-xl transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl`}>
            {/* Header / Banner */}
            ...
            <div className="p-8">
                ...
                <div className="flex flex-wrap items-center gap-3 mb-6">
                    ...
                    <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 dark:bg-white/5 text-ink-muted text-[10px] font-black uppercase tracking-wider">
                ...
                {/* Title & Insight */}
                <div className="mb-8">
                    <h3 className="text-2xl font-black text-ink mb-4 leading-tight group-hover:text-primary-soft transition-colors duration-300">
                        {opportunity.title}
                    </h3>
                    <p className="text-ink-muted leading-relaxed text-lg font-medium">
                        {opportunity.insight}
                    </p>
                </div>

                {/* Metrics Breakdown */}
                <div className="grid grid-cols-2 gap-4 mb-8 bg-slate-50 dark:bg-white/5 p-6 rounded-2xl border border-line/80 dark:border-line/10">
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                        {opportunity.actions.map((action, idx) => (
                            <div key={idx} className="flex items-start gap-3 p-4 bg-surface rounded-xl border border-line/80 dark:border-line/10 shadow-sm group/action transition-all hover:border-primary/30 hover:bg-slate-50 dark:hover:bg-slate-800/80">
                                ...
                                <span className="text-sm font-semibold text-ink-muted leading-snug">
                                    {action}
                                </span>
                            </div>
                        ))}
            ...
            <div className="absolute -bottom-4 -right-4 w-24 h-24 bg-primary/5 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-700" />
