    ...
    return (
        <div className="w-full h-full flex flex-col gap-4 overflow-hidden">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="flex gap-1.5 flex-wrap">
                            <button
                                ...
                                className={`px-4 py-2 rounded-xl border text-[9px] font-black uppercase tracking-widest transition-all duration-300 ${referenceKey === 'AVERAGE' ? 'bg-primary text-white shadow-lg shadow-primary/20' : (isDark ? 'bg-white/5 border-white/10 text-slate-400' : 'bg-white border-slate-200 text-slate-500')}`}
                            >
                                Overall Average
                            </button>
                            {brands.map((brand) => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-xl border text-[9px] font-black uppercase tracking-widest transition-all duration-300 ${referenceKey === brand ? 'bg-primary text-white shadow-lg shadow-primary/20' : (isDark ? 'bg-white/5 border-white/10 text-slate-400' : 'bg-white border-slate-200 text-slate-500')}`}
                                >
                                    {brand}
                                </button>
                            ))}
                    ...
                    <div className="flex-1 flex flex-col gap-2 hidden md:flex">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2">
                                <span className="text-[10px] font-black text-primary-soft uppercase italic">{referenceLabel}</span>
            ...
            >
                ...
                <div style={gridStyle} className={`px-8 py-4 text-[9px] font-black uppercase tracking-[0.3em] ${isDark ? 'text-slate-500' : 'text-slate-900'} border-b ${isDark ? 'border-white/10' : 'border-slate-200'} sticky top-0 ${isDark ? 'bg-slate-900' : 'bg-white'} z-30 shrink-0`}>
                    ...
                    <div className={`sticky z-10 text-center ${isDark ? 'bg-slate-900' : 'bg-white'} px-2`} style={{ left: stickyCol1Width }}>
                        <div className="bg-primary text-white rounded-lg py-1.5 shadow-lg shadow-primary/20 w-full">
                        ...
                        return (
                            ...
                            >
                                <div className={`flex items-center gap-3 py-2 sticky left-0 z-10 ${isDark ? (rowIndex % 2 === 0 ? 'bg-[#0d1525]' : 'bg-[#0f172a]') : (rowIndex % 2 === 0 ? 'bg-[#fcfdfe]' : 'bg-white')} pr-4 transition-colors`}>
                                    <div className={`p-1.5 rounded-lg ${isDark ? 'bg-white/5' : 'bg-slate-100'} group-hover:scale-110 group-hover:bg-primary/10 transition-all`}>
                                        <BarChart2 className="h-3.5 w-3.5 text-primary-soft" />
                                    </div>
                                ...
                                <div className={`sticky z-10 text-center font-mono text-sm font-black ${isDark ? 'text-white' : 'text-slate-900'} py-2 my-1 px-2 ${isDark ? (rowIndex % 2 === 0 ? 'bg-[#0d1525]' : 'bg-[#0f172a]') : (rowIndex % 2 === 0 ? 'bg-[#fcfdfe]' : 'bg-white')} transition-colors`} style={{ left: stickyCol1Width }}>
                                    <div className="bg-primary/5 rounded-xl py-1 w-full h-full flex items-center justify-center border border-primary/10">
            ...
            <div className={`flex items-center justify-between ${isFocusMode ? 'px-8 py-3' : 'px-10 py-5'} rounded-[24px] border ${isDark ? 'bg-white/[0.02] border-white/5' : 'bg-slate-50 border-slate-200'} shrink-0`}>
                ...
                {!isFocusMode && (
                    <div className="text-[9px] font-black uppercase italic text-primary-soft tracking-[0.2em] opacity-50">
