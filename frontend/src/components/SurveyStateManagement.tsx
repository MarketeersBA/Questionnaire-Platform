...
const STATUS_CONFIG: Record<string, { label: string; dot: string; text: string; bg: string; border: string }> = {
    ...
    closed: { label: 'Closed', dot: 'bg-slate-400', text: 'text-ink-muted', bg: 'bg-surface-raised/50', border: 'border-slate-200 dark:border-slate-700/50' },
};
                ...
                {isOpen && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="absolute left-0 mt-2 w-44 bg-surface rounded-2xl border border-line/80 dark:border-line/10 shadow-xl z-40 overflow-hidden"
                        >
                ...
                {isConfirming && (
                    ...
                    >
                        <motion.div
                            ...
                            className="bg-surface rounded-[2rem] p-8 max-w-sm w-full border border-line/80 dark:border-line/10 shadow-2xl relative"
                        >
                            {/* Close button */}
                            <button
                                onClick={() => setIsConfirming(null)}
                                className="absolute top-4 right-4 w-8 h-8 rounded-xl bg-surface-raised text-ink-subtle hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white transition-colors flex items-center justify-center"
                            >
                                <X size={14} />
                            </button>
                            ...
                            <p className="text-sm text-ink-muted text-center font-medium leading-relaxed mb-2">
                                Moving from <span className="font-black text-ink-muted uppercase text-xs">{currentStatus}</span> → <span className="font-black text-ink-muted uppercase text-xs">{isConfirming}</span>
                            </p>
                            {isConfirming === 'active' && (
                                <p className="text-[11px] text-ink-subtle text-center mb-6">
                            ...
                            <div className="grid grid-cols-2 gap-3 mt-4">
                                <button
                                    onClick={() => setIsConfirming(null)}
                                    disabled={loading}
                                    className="px-4 py-3 rounded-xl bg-surface-raised text-ink-muted font-black text-xs hover:bg-slate-100 dark:hover:bg-slate-700 transition-all border border-slate-200 dark:border-slate-700 disabled:opacity-50"
                                >
                                    Cancel
                                </button>
                                <button
                                    ...
                                    className={`px-4 py-3 rounded-xl text-white font-black text-xs transition-all flex items-center justify-center gap-2 disabled:opacity-50 ${isConfirming === 'active'
                                        ...
                                        : 'bg-slate-900 dark:bg-primary dark:hover:bg-brand-blueHover hover:bg-slate-700 shadow-lg shadow-slate-900/10 dark:shadow-primary/20'
                                        }`}
                                >
                                    {loading
