...
export default function BrandAnalyzerGrid({
    ...
    return (
        <div className="w-full h-full flex flex-col gap-4">
            ...
            <div className="relative glass-panel rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-2xl overflow-hidden bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl">

                {/* 1. Header Section - Locked to top */}
                <div
                    className="sticky top-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 grid"
                    style={{ gridTemplateColumns: `minmax(120px, 200px) repeat(${applicableBrands.length}, minmax(0, 1fr))` }}
                >
                    <div className="p-5 text-start font-black uppercase tracking-[0.2em] text-[10px] text-brand-blue border-r border-slate-50 dark:border-slate-800/50 sticky left-0 z-50 bg-inherit">
                        {isAr ? 'الصفات' : 'Attributes'}
                    </div>
                    {applicableBrands.map(brand => (
                        <div key={brand} className="p-3 flex items-center justify-center text-center font-black uppercase tracking-tight text-[10px] md:text-[11px] text-slate-500 dark:text-slate-400 border-r border-slate-50 dark:border-slate-800/50 last:border-r-0">
                            {brand}
                        </div>
                    ))}
                </div>
                ...
                <div className="max-h-[500px] overflow-y-auto custom-scrollbar">
                    {rows.map((row: any, rIdx: number) => {
                        ...
                        return (
                            <div
                                key={attrId}
                                className={`grid group hover:bg-brand-blue/[0.02] transition-colors border-b border-slate-50 dark:border-slate-800/40 ${isLastRow ? 'border-b-0' : ''}`}
                                style={{ gridTemplateColumns: `minmax(120px, 200px) repeat(${applicableBrands.length}, minmax(0, 1fr))` }}
                            >
                                ...
                                {applicableBrands.map((brand) => {
                                    ...
                                    return (
                                        <div
                                            key={brand}
                                            className={`p-1 flex items-center justify-center cursor-pointer transition-all border-r border-slate-50 dark:border-slate-800/50 last:border-r-0 ${isSelected ? 'bg-brand-blue/[0.03]' : ''}`}
                                        ...
                                        >
                                            <motion.div
                                                ...
                                                className={`w-5 h-5 md:w-6 md:h-6 rounded-lg border-2 flex items-center justify-center transition-all ${isSelected
                                                    ...
                                                    : 'bg-transparent border-slate-200 dark:border-slate-700/50 group-hover:border-brand-blue/30'
                                                    }`}
                                            >
